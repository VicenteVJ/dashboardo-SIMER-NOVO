...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="card"><div><h2 className="font-bold">Qualidade da base</h2><p className="mt-0.5 text-xs text-muted">Alertas calculados no recorte visível, sem exigir novas colunas no Excel.</p></div><MetricGrid variant="mini" className="mt-3">{[['Sem departamento', visibleSummary.qualidade.semDepartamento], ['Sem responsável', visibleSummary.qualidade.semResponsavel], ['Sem categoria', visibleSummary.qualidade.semCategoria], ['Sem cliente', visibleSummary.qualidade.semCliente], ['Sem última ação', visibleSummary.qualidade.semUltimaAcao], ['Status desconhecido', visibleSummary.qualidade.statusDesconhecido]].map(([label, value]) => <div key={label} className="flex min-h-[72px] items-center justify-between gap-2 rounded-lg bg-slate-50 p-3 dark:bg-slate-800"><p className="text-[11px] leading-4 text-muted">{label}</p><strong className="shrink-0 text-xl leading-none">{value}</strong></div>)}</MetricGrid></section>
      <section className="card"><div className="flex items-center justify-between"><div><h2 className="font-bold">Aging de tickets abertos</h2><p className="mt-0.5 text-xs text-muted">Clique em uma faixa para isolar os tickets.</p></div>{filters.aging && <button className="text-xs font-bold text-brand" onClick={() => setFilters((current) => ({ ...current, aging: '' }))}>Limpar aging</button>}</div><MetricGrid variant="aging" className="mt-3">{[['0–3 dias', visibleSummary.aging.zeroATres, '0-3'], ['4–7 dias', visibleSummary.aging.quatroASete, '4-7'], ['8–15 dias', visibleSummary.aging.oitoAQuinze, '8-15'], ['+15 dias', visibleSummary.aging.maisDeQuinze, '15+']].map(([label, value, aging]) => <button key={aging} onClick={() => setFilters((current) => ({ ...current, aging: current.aging === aging ? '' : aging }))} className={`min-h-[80px] rounded-lg border p-3 text-left transition hover:border-brand ${filters.aging === aging ? 'border-brand bg-blue-50 dark:bg-blue-950' : 'bg-slate-50 dark:bg-slate-800'}`}><strong className="block text-2xl leading-none">{value}</strong><span className="mt-1 block text-xs leading-4 text-muted">{label}</span></button>)}</MetricGrid></section>
