...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-4">
      <section><p className="text-[10px] font-extrabold uppercase tracking-[.16em] text-brand">Visão operacional</p><h1 className="mt-1 text-2xl font-extrabold sm:text-3xl">Tickets ERP Simer</h1><p className="mt-1.5 max-w-4xl text-xs leading-5 text-muted">Foco em gestão: backlog, criticidade, espera e tendência mensal com detalhamento.</p></section>
      ...
      <section>
        <div className="mb-2.5 flex min-h-8 items-center justify-between gap-3">
          <div><h2 className="font-bold">Indicadores</h2><p className="mt-0.5 text-xs text-muted">Clique em um card para aplicar o recorte.</p></div>
          {(filters.categoria || filters.categoriaGrupo) && <button className="btn-secondary px-3 py-2 text-xs" onClick={() => setFilters((current) => ({ ...current, categoria: '', categoriaGrupo: '' }))}><span aria-hidden="true">×</span> Limpar categoria</button>}
        </div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{cards.map(([id, label, value, note, tone]) => <StatCard key={label} label={label} value={value} note={note} tone={tone} active={filters.indicador === id && Boolean(id)} onClick={() => id ? setIndicator(id) : setFilters((current) => ({ ...current, indicador: '' }))} />)}<StatCard label="Categoria: Problema" value={cardSummary.problemas} note="Ocorrências classificadas" tone="slate" active={filters.categoriaGrupo === 'problema'} onClick={() => setCategoryGroup('problema')} /><StatCard label="Adequação / Dúvida" value={cardSummary.adequacaoDuvida} note="Demandas consultivas" tone="blue" active={filters.categoriaGrupo === 'adequacao-duvida'} onClick={() => setCategoryGroup('adequacao-duvida')} /></div>
      </section>
      <section>
        <div className="mb-2.5 flex items-center justify-between gap-3"><div><h2 className="font-bold">Governança, ITIL e risco</h2><p className="mt-0.5 text-xs text-muted">Indicadores inferidos pelo sistema; o Excel original não é alterado.</p></div>{filters.governanca && <button className="btn-secondary px-3 py-2 text-xs" onClick={() => setFilters((current) => ({ ...current, governanca: '' }))}>× Limpar governança</button>}</div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{governanceCards.map(([id, label, value, note, tone]) => <StatCard key={id} inferred label={label} value={value} note={note} tone={tone} active={filters.governanca === id} onClick={() => setGovernance(id)} />)}</div>
      </section>
      <section className="card"><div><h2 className="font-bold">Qualidade da base</h2><p className="mt-0.5 text-xs text-muted">Alertas calculados no recorte visível, sem exigir novas colunas no Excel.</p></div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">{[['Sem departamento', visibleSummary.qualidade.semDepartamento], ['Sem responsável', visibleSummary.qualidade.semResponsavel], ['Sem categoria', visibleSummary.qualidade.semCategoria], ['Sem cliente', visibleSummary.qualidade.semCliente], ['Sem última ação', visibleSummary.qualidade.semUltimaAcao], ['Status desconhecido', visibleSummary.qualidade.statusDesconhecido]].map(([label, value]) => <div key={label} className="flex min-h-[72px] items-center justify-between gap-2 rounded-lg bg-slate-50 p-3 dark:bg-slate-800"><p className="text-[11px] leading-4 text-muted">{label}</p><strong className="shrink-0 text-xl leading-none">{value}</strong></div>)}</div></section>
      <section className="card"><div className="flex items-center justify-between"><div><h2 className="font-bold">Aging de tickets abertos</h2><p className="mt-0.5 text-xs text-muted">Clique em uma faixa para isolar os tickets.</p></div>{filters.aging && <button className="text-xs font-bold text-brand" onClick={() => setFilters((current) => ({ ...current, aging: '' }))}>Limpar aging</button>}</div><div className="mt-3 grid gap-2 sm:grid-cols-4">{[['0–3 dias', visibleSummary.aging.zeroATres, '0-3'], ['4–7 dias', visibleSummary.aging.quatroASete, '4-7'], ['8–15 dias', visibleSummary.aging.oitoAQuinze, '8-15'], ['+15 dias', visibleSummary.aging.maisDeQuinze, '15+']].map(([label, value, aging]) => <button key={aging} onClick={() => setFilters((current) => ({ ...current, aging: current.aging === aging ? '' : aging }))} className={`min-h-[80px] rounded-lg border p-3 text-left transition hover:border-brand ${filters.aging === aging ? 'border-brand bg-blue-50 dark:bg-blue-950' : 'bg-slate-50 dark:bg-slate-800'}`}><strong className="block text-2xl leading-none">{value}</strong><span className="mt-1 block text-xs leading-4 text-muted">{label}</span></button>)}</div></section>
