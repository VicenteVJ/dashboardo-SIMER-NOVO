...
export default function ExecutivePage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid gap-5 xl:grid-cols-[1.25fr_1fr]"><div className="card"><h2 className="font-bold">Sinais do backlog</h2><p className="mt-1 text-xs text-muted">Percentuais sobre {categoryScope.length} ticket(s) abertos.</p><div className="mt-6 space-y-6"><ProgressMetric label="Criticidade P1/P2" value={metrics.critical} total={categoryScope.length} tone="bg-red-500" note="Prioridade gerencial inferida" /><ProgressMetric label="SLA vencido" value={metrics.sla} total={categoryScope.length} tone="bg-orange-500" note="Comparação com a referência P1–P4" /><ProgressMetric label="Dependência externa" value={metrics.external} total={categoryScope.length} tone="bg-violet" note="Responsável ou contexto externo" /></div></div><div className="card"><h2 className="font-bold">Idade e movimentação</h2><p className="mt-1 text-xs text-muted">Médias do backlog selecionado.</p><div className="mt-6 grid gap-3 sm:grid-cols-3 xl:grid-cols-1 2xl:grid-cols-3"><div className="rounded-2xl bg-blue-50 p-5 dark:bg-blue-950/50"><p className="text-[10px] font-bold uppercase text-muted">Tempo em aberto</p><strong className="mt-2 block text-3xl">{metrics.averageAge}<span className="ml-1 text-sm">dias</span></strong></div><div className="rounded-2xl bg-amber-50 p-5 dark:bg-amber-950/40"><p className="text-[10px] font-bold uppercase text-muted">Sem atualização</p><strong className="mt-2 block text-3xl">{metrics.averageStale}<span className="ml-1 text-sm">dias</span></strong></div><div className="rounded-2xl bg-violet/10 p-5"><p className="text-[10px] font-bold uppercase text-muted">Média externa</p><strong className="mt-2 block text-3xl">{metrics.externalAverageStale}<span className="ml-1 text-sm">dias</span></strong></div></div></div></section>
      ...
      <section className="card border-indigo-200"><div className="flex items-center gap-2"><h2 className="font-bold">Leitura executiva por Eisenhower e Pareto</h2><span className="cursor-help text-violet" title="Indicadores calculados automaticamente sobre o recorte atual.">ⓘ</span></div><div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-2xl bg-red-50 p-4 dark:bg-red-950/40"><p className="text-[10px] font-bold uppercase text-muted">Fazer agora</p><strong className="mt-2 block text-3xl">{metrics.doNow}</strong><span className="text-xs text-muted">{categoryScope.length ? ((metrics.doNow / categoryScope.length) * 100).toFixed(1) : 0}% do backlog</span></div><div className="rounded-2xl bg-blue-50 p-4 dark:bg-blue-950/40"><p className="text-[10px] font-bold uppercase text-muted">Clientes Pareto</p><strong className="mt-2 block text-3xl">{paretoStats.clients}</strong><span className="text-xs text-muted">concentram ~80%</span></div><div className="rounded-2xl bg-violet/10 p-4"><p className="text-[10px] font-bold uppercase text-muted">Serviços Pareto</p><strong className="mt-2 block text-3xl">{paretoStats.services}</strong><span className="text-xs text-muted">grupos prioritários</span></div><div className="rounded-2xl bg-amber-50 p-4 dark:bg-amber-950/40"><p className="text-[10px] font-bold uppercase text-muted">Categorias Pareto</p><strong className="mt-2 block text-3xl">{paretoStats.categories}</strong><span className="text-xs text-muted">maior concentração</span></div></div><div className="mt-5 grid gap-3 text-sm leading-6 lg:grid-cols-2"><p>• Dos <strong>{categoryScope.length} tickets abertos</strong>, <strong>{metrics.doNow}</strong> estão no quadrante Fazer agora.</p><p>• O maior foco deve estar no serviço <strong>{topServices[0]?.name || 'não identificado'}</strong>, com {topServices[0]?.total || 0} ticket(s).</p><p>• O cliente com maior demanda é <strong>{topClients[0]?.name || 'não identificado'}</strong>, com {topClients[0]?.total || 0} ticket(s).</p><p>• Para reduzir maior volume, priorize os ofensores Pareto de cliente, serviço e categoria e os tickets Fazer agora.</p></div></section>
