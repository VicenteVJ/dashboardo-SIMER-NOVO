    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex w-[220px] flex-col overflow-hidden bg-slate-950 px-3 py-3 text-white transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="mt-3 space-y-0.5">
        {navigation.map(([to, icon, label]) => (
          <NavLink key={to} to={to} end={to === '/'} onClick={onClose} className={({ isActive }) => `flex items-center gap-2 rounded-lg px-2.5 py-1.5 text-xs font-semibold transition ${isActive ? 'bg-white text-slate-950' : 'text-slate-300 hover:bg-white/10 hover:text-white'}`}>
      ...
      <div className="mt-3">
        ...
        <div className="mt-1.5 grid grid-cols-2 gap-1.5">
          {[
            ...
            <div key={label} className="rounded-lg border border-white/10 bg-white/5 p-1.5">
      ...
      <div className="mt-3 flex min-h-0 flex-1 flex-col">
        ...
        <div className="thin-scrollbar mt-1.5 min-h-0 flex-1 space-y-1 overflow-y-auto pr-1">
          {prioritized.length ? prioritized.map((ticket) => (
            <NavLink to={`/ticket?numero=${encodeURIComponent(ticket.numero)}`} key={ticket.numero} className="block rounded-lg border border-white/10 bg-white/5 p-1.5 hover:bg-white/10">
